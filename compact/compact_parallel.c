...
int main(int argc, char *argv[]){
	if(argc != 3) {
		printf("Usage:\nbash$:./compact_parallel <sizeOfData> <debugFlag>\n\n");
		printf("Example: \n"); 
		printf("$bash: ./compact_parallel 10 1\n"); 
		printf("That generates 10 random character and Turns on Debug\n"); 
		printf("$bash: ./compact_parallel 10 0\n"); 
...
// PARALLALIZE FROM HERE
	#pragma omp parallel shared(index,tmp_ind,new_index,comp,data, chunk) private(i)
	{
